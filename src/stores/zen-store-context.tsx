...
import { useZenGlobalStore } from "./global-store";
import { useZenPanelStore } from "./panel-store";
import { useZenPanelUiStore } from "./panel-ui-store";
import type { ZenGlobalState } from "./global-store";
import type { ZenPanelState } from "./panel-store";
import type { ZenPanelUiState } from "./panel-ui-store";
...
export const ZenStoreProvider: React.FC<{
  ...
  const stores = useMemo(() => ({ ...defaultStores, ...overrideStores }), [overrideStores]);

  return <ZenStoreContext.Provider value={stores}>{children}</ZenStoreContext.Provider>;
};
...
export function useZenWindowStoreContext<T>(selector: (state: ZenWindowStoreState) => T): T {
  const { windowStore } = useContext(ZenStoreContext);
  return windowStore(selector);
}

export function useZenPanelStoreContext<T>(selector: (state: ZenPanelState) => T): T {
  const { panelStore } = useContext(ZenStoreContext);
  return panelStore(selector);
}

export function useZenGlobalStoreContext<T>(selector: (state: ZenGlobalState) => T): T {
  const { globalStore } = useContext(ZenStoreContext);
  return globalStore(selector);
}

export function useZenPanelUiStoreContext<T>(selector: (state: ZenPanelUiState) => T): T {
